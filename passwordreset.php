...
if (isset($_GET["key"]) && (isset($_GET["email"]) && (isset($_GET["action"]) 
 && ($_GET["action"]=="reset") && isset($_POST["action"])){
    ...
    deactivated.</p>
    <p><a href="https://localhost/connect_me/password_reset/.php">
...
&& ($_GET["action"]=="reset") && !isset($_POST["action"])){
  ...
  $username = $_GET["email"];
  $query = mysqli_query($conn,
  "SELECT * FROM `password_reset` WHERE `key`='".$key."' and `username`='".$username."';"
  );
  $row = mysqli_num_rows($query);
  if ($row==""){
  $error .= '<h2>Invalid Link</h2>
...
from the email, or you have already used the key in which case it is 
deactivated.</p>
<p><a href="passwordreset/index.php">
Click here</a> to reset password.</p>';
}else{
    $row = mysqli_fetch_assoc($query);
    if ($id >= $curDate){
...
mysqli_query($conn,"DELETE FROM `password_reset` WHERE `email`='".$username."';");
 
echo '<div class="error"><p>Congratulations! Your password has been updated successfully.</p>
<p><a href="passwordreset/login.php">
Click here</a> to Login.</p></div><br />';
   } 
}
?>